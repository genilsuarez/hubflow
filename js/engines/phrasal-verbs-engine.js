...
import { finishExercise, advanceStudyCard, createMatchMode, squeezeToggle, showStudyFollowUpOverlay, handleStudyKeydown, handleQuizNextKeydown, findNextPendingCategory } from '../exercise-flow.js';
...
export function initPhrasalVerbs({ categories, scoreKeyPrefix }) {
  ...
  function findStudyFollowUp() {
    ...
    const cat = findNextPendingCategory({
      ...
      isPending: c => [`${scoreKeyPrefix}-${c}-quiz`, `${scoreKeyPrefix}-${c}-timed`].some(k => !getScoreStatus(k).passed),
    });
    if (cat) {
