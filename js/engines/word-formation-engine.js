/* ═══════════════════════════════════════════════════════
   ...
   Study / Quiz / Timed: escribir la forma derivada a partir de la raíz.
...
import { finishExercise, squeezeToggle, showStudyFollowUpOverlay, handleStudyKeydown, findNextPendingCategory, createStudyNav } from '../exercise-flow.js';
...
export function initWordFormation({ categories, scoreKeyPrefix }) {
  ...
  function skipQuiz() { if (answered || idx >= total) return; idx++; updProgress(idx, total); renderQuiz(); }
  function startMode() { stopTimer(); document.querySelectorAll('[data-area]').forEach(a => a.classList.remove('show')); document.getElementById('timerBar').classList.remove('show'); setQuizAnswered(false); if (mode === 'quiz') initQuiz(false); else if (mode === 'timed') initQuiz(true); else initStudy(); }
  ...
  function renderQuiz() { if (idx >= total) { stopTimer(); finishQuiz(); return; } answered = false; setQuizAnswered(false); const item = deck[idx]; document.getElementById('scIcon').textContent = categories[currentCat].icon; document.getElementById('scText').innerHTML = blankHTML(item.sentence, '?'); document.getElementById('scCounter').textContent = `${idx+1} / ${total}`; document.getElementById('explainBox').textContent = ''; document.getElementById('rootWord').textContent = `Root: ${item.root}`; const input = document.getElementById('answerInput'); input.value = ''; input.className = 'answer-input'; input.disabled = false; input.focus(); updProgress(idx, total); }
  function checkAnswer() { if (answered) return; const input = document.getElementById('answerInput'); const val = input.value.trim().toLowerCase(); const item = deck[idx]; if (!val) return; answered = true; input.disabled = true; const correct = val === item.correct.toLowerCase(); if (correct) { input.classList.add('correct'); score++; } else { input.classList.add('wrong'); } document.getElementById('scText').innerHTML = blankHTML(item.sentence, item.correct); document.getElementById('explainBox').textContent = item.explain; idx++; updProgress(idx, total); const nextBtn = document.getElementById('quizNextBtn'); if (nextBtn) { nextBtn.textContent = idx >= total ? 'Ver resultado →' : 'Siguiente →'; setQuizAnswered(true); nextBtn.onclick = () => renderQuiz(); nextBtn.focus({ preventScroll: true }); } else { setTimeout(renderQuiz, 1600); } }
