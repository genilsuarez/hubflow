/* ═══════════════════════════════════════════════════════
   ...
   Study / Quiz / Timed de tiempos verbales (opciones por item o por categoría).
...
export function initTenses({ categories, scoreKeyPrefix }) {
  ...
  // Mirrors sentence-quiz-engine: quiz key = ${prefix}-${cat}, timed = ${prefix}-${cat}-timed.
  function findStudyFollowUp() {
    ...
    for (const m of MODES) {
      if (!getScoreStatus(m.key(currentCat)).passed) {
        return { label: m.label, isNewCategory: false, onContinue: () => { mode = m.suffix ? 'timed' : 'quiz'; syncModeTabsActive(mode); startMode(); } };
      }
    }
