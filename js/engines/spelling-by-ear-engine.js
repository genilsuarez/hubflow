/* ═══════════════════════════════════════════════════════
   ...
   Practice / Timed: se escucha la palabra (TTS) y se escribe.
...
export function initSpellingByEar({ categories, scoreKeyPrefix }) {
  ...
  let currentCat = Object.keys(categories)[0], mode = 'practice', deck = [], idx = 0, score = 0, total = 0, timer = null, answered = false;
  sharedRenderCatBar({
  ...
  function skipPractice() { if (answered || idx >= total) return; idx++; updProgress(idx, total); renderItem(); }
  function startMode() { stopTimer(); document.querySelectorAll('[data-area]').forEach(a => a.classList.remove('show')); document.getElementById('timerBar').classList.remove('show'); setQuizAnswered(false); initPractice(mode === 'timed'); }
  ...
  function finishPractice() { const pct = finishExercise({ correct: score, total, startMode }); const _timedSuffix = mode === 'timed' ? '-timed' : ''; recordScore(`${scoreKeyPrefix}-${currentCat}${_timedSuffix}`, pct); }
  startMode();
}
